...
var arrayJoinMatrixPath = moduleRootPath("testdata", "clickhouse-array-join-matrix.json")

func TestArrayJoinMatrixAgainstChgen(t *testing.T) {
	...
	for _, cell := range artifact.Cells {
		t.Run(strings.ReplaceAll(cell.ID, "/", "_"), func(t *testing.T) {
			results, err := inferQueryResults(conformance.ArrayJoinFixtureDDL, cell.Query)
...
func TestPinnedArrayJoinMatrixIsCurrent(t *testing.T) {
	wantArtifact := conformance.CurrentArrayJoinArtifact(MeasuredCHVersion)
	want, err := json.MarshalIndent(wantArtifact, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	want = append(want, '\n')
	path := filepath.Clean(arrayJoinMatrixPath)
	if os.Getenv("CHGEN_UPDATE_ARRAY_JOIN_MATRIX") == "1" {
	...
	var gotArtifact conformance.ArrayJoinArtifact
	if err := json.Unmarshal(got, &gotArtifact); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(gotArtifact, wantArtifact) {
		t.Fatal("ARRAY JOIN matrix artifact is stale; run with CHGEN_UPDATE_ARRAY_JOIN_MATRIX=1")
	}
}
