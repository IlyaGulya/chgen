...
type Querier interface {
	InsertAuditRow(ctx context.Context, arg InsertAuditRowParams) error
}
...
type MockQuerier struct {
	InsertAuditRowFunc func(ctx context.Context, arg InsertAuditRowParams) error
}

var _ Querier = (*Queries)(nil)
var _ Querier = (*MockQuerier)(nil)

func (m *MockQuerier) InsertAuditRow(ctx context.Context, arg InsertAuditRowParams) error {
	if m == nil || m.InsertAuditRowFunc == nil {
		return fmt.Errorf("MockQuerier.InsertAuditRowFunc is nil")
	}
	return m.InsertAuditRowFunc(ctx, arg)
}
...
type InsertAuditRowParams struct {
	ID        uint64
	Actor     string
	Note      *string
	WrittenAt time.Time
...
const insertAuditRowSQL = `INSERT INTO audit_log (id, actor, note, written_at)
VALUES (?, ?, ?, ?);`

// insertAuditRowBatchSQL is the prefix that PrepareBatch takes. The native batch
// path keeps the sub-second fraction of a time.Time that the client-side text
// interpolation of conn.Exec drops.
const insertAuditRowBatchSQL = "INSERT INTO audit_log (id, actor, note, written_at)"

// InsertAuditRow executes the annotated ClickHouse statement.
func (q *Queries) InsertAuditRow(ctx context.Context, arg InsertAuditRowParams) error {
	if ctx == nil {
		ctx = context.Background()
	}
	if err := chgenGuardDateTime64(arg.WrittenAt, "WrittenAt"); err != nil {
		return fmt.Errorf("InsertAuditRow: %w", err)
	}
	batch, err := q.conn.PrepareBatch(ctx, insertAuditRowBatchSQL)
	if err != nil {
		return fmt.Errorf("InsertAuditRow prepare batch: %w", err)
	}
	if err := batch.Append(arg.ID, arg.Actor, arg.Note, arg.WrittenAt); err != nil {
		return fmt.Errorf("InsertAuditRow append: %w", err)
	}
	if err := batch.Send(); err != nil {
		return fmt.Errorf("InsertAuditRow send: %w", err)
	}
	return nil
}
