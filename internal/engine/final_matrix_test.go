...
var finalMatrixPath = moduleRootPath("testdata", "clickhouse-final-matrix.json")

func TestFinalMatrixAgainstChgen(t *testing.T) {
	...
	for _, cell := range artifact.Cells {
		t.Run(strings.ReplaceAll(cell.ID, "/", "_"), func(t *testing.T) {
			results, err := inferQueryResults(conformance.FinalFixtureDDL, cell.Query)
...
func TestPinnedFinalMatrixIsCurrent(t *testing.T) {
	wantArtifact := conformance.CurrentFinalArtifact(MeasuredCHVersion)
	want, err := json.MarshalIndent(wantArtifact, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	want = append(want, '\n')
	path := filepath.Clean(finalMatrixPath)
	if os.Getenv("CHGEN_UPDATE_FINAL_MATRIX") == "1" {
	...
	var gotArtifact conformance.FinalArtifact
	if err := json.Unmarshal(got, &gotArtifact); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(gotArtifact, wantArtifact) {
		t.Fatal("FINAL matrix artifact is stale; run with CHGEN_UPDATE_FINAL_MATRIX=1")
	}
}
