...
`)
	_, err := ParseSchemaCatalogs([]string{path})
	want := path + ":2: statement is not CREATE TABLE, DROP TABLE/VIEW, RENAME TABLE, EXCHANGE TABLES, or a supported ALTER TABLE; chgen does not model this schema operation"
	if err == nil || err.Error() != want {
		t.Fatalf("got %v, want %q", err, want)
	}
}
...
func TestParseSchemaCatalogsRejectsNonCatalogDropStatements(t *testing.T) {
	...
	for name, ddl := range tests {
		t.Run(name, func(t *testing.T) {
			...
			_, err := ParseSchemaCatalogs([]string{path})
			want := path + ":1: statement is not CREATE TABLE, DROP TABLE/VIEW, RENAME TABLE, EXCHANGE TABLES, or a supported ALTER TABLE; chgen does not model this schema operation"
			if err == nil || err.Error() != want {
				t.Fatalf("got %v, want %q", err, want)
			}
		})
