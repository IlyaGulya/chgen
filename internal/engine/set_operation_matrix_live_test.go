...
// TestSetOperationMatrixAgainstClickHouse keeps analysis and execution as
// separate witnesses for every set-query and CTE boundary.
func TestSetOperationMatrixAgainstClickHouse(t *testing.T) {
	ddl := setOperationFixtureParts(conformance.SetOperationFixtureDDL)
	seed := setOperationFixtureParts(conformance.SetOperationFixtureSeed)
	if len(ddl) == 0 || len(seed) == 0 {
		t.Fatal("set-operation fixture is empty")
	}
	...
	for _, cell := range artifact.Cells {
		t.Run(strings.ReplaceAll(cell.ID, "/", "_"), func(t *testing.T) {
			analysis, analysisErr := setOperationServerResults(oracle, cell.Query)
...
func setOperationFixtureParts(sql string) []string {
	var parts []string
...
type setOperationDescribeResult struct {
	Name string `json:"name"`
	Type string `json:"type"`
}

func setOperationServerResults(oracle *chOracle, query string) ([]setOperationDescribeResult, error) {
	raw, err := oracle.exec("DESCRIBE TABLE (" + query + ") FORMAT JSONEachRow")
	if err != nil {
		return nil, err
	}
	var results []setOperationDescribeResult
	for _, line := range strings.Split(strings.TrimSpace(raw), "\n") {
		var result setOperationDescribeResult
		if err := json.Unmarshal([]byte(line), &result); err != nil {
			return nil, fmt.Errorf("decode set-operation analysis: %w", err)
		}
...
func setOperationErrorHasCode(err error, code int) bool {
